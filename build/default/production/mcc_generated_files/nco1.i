...
void SetFrequency(uint16_t freq)
{
    ...
    if(freq>300000U) return;

    NCO1CONbits.EN = 0;
    uint32_t inc = (uint32_t)(((uint64_t)freq << 21) / NCO1_CLK_HZ);
    NCO1INCU = (uint8_t)((inc >> 16) & 0xFF);
...
uint32_t GetFrequency(void){
    return ((uint32_t)NCO1INCU << 16) | ((uint32_t)NCO1INCH << 8) | (uint32_t)NCO1INCL;
}
